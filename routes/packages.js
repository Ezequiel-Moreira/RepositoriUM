...
//portanto, agora temos de retirar o codigo que está dentro dos router.get e router.post,
//colocar numa função à parte e depois chamar em 2 sitios, com os routers apropriados
//e repetir isto para todos os get e post que temos
// não, não todos. Apenas para a listagem de pacotes e para os detalhes de pacotes
//ok
...
//uma coisa: é preciso 2 gets?
// sim
routerApi.get('/',(req,res,nest)=>{
	packagesList(req,res,(err,data)=>{
		// Este caso vai ser um bocado diferente, porque este é a API
		// Vamos fazer de conta que o formato a retornar é: { error: ... }, ou em caso de sucessor: { error: null, data: data.packages }
		if(err){
...
// ok, e agora temos de fazer a mesma coisa para o get( '/:id' )
//ok
...
routerApi.get('/:id',(req,res,next)=>{
	packagesDetails(req,res,(err,data)=>{
		 ...
		 else{res.json({error:null,data:{...data.package.toObject(),abstractHTML:data.abstract}});}
	});
});
